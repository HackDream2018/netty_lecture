...
public class DateUtils {
	...
	{
		return getFormatDate(dt, DEFAULT_DATE_FORMAT);
	}
	...
	{
		return getFormatDate(dt, DATE_FORMAT_DATE_TIME);
	}
	 ...
	 * @return String
	 */
	public static boolean inTimeSegment2(Date start, Date end, Date date)
	{
		if (date.after(start) && date.before(end))
		{
			return true;
	...
	{
		return new Date(date.getTime() + num * (long) 60 * 1000);
	}
    ...
    public static String getPreMonthDate()
    {
    	Calendar c = Calendar.getInstance();
    	c.add(Calendar.MONTH,- 1);
    	return getFormatDate(c.getTime(), "yyyyMMdd");
    }
}
